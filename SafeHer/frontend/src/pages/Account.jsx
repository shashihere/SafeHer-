...
const Account = () => {
    ...
    return (
        <div className="bg-transparent min-h-[calc(100vh-80px)] text-slate-800 pt-24 pb-20 px-4">
            <div className="max-w-5xl mx-auto">
                <div className="mb-16">
                    <h1 className="text-5xl md:text-7xl font-cursive mb-6">Your Sanctuary</h1>
                    <p className="text-slate-600 font-sans uppercase tracking-widest text-sm border-b border-slate-200 pb-8">
                    ...
                    <div className="md:col-span-1 border border-gray-300 bg-white/50 p-4 space-y-2 font-sans tracking-widest uppercase text-xs font-bold flex flex-col justify-between" style={{ minHeight: '300px' }}>
                        <div className="space-y-2">
                            <button 
                                ...
                                className={`w-full text-left px-6 py-4 flex items-center gap-3 transition-all ${activeTab === 'profile' ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-white text-slate-800 border border-slate-200 hover:border-blue-600 hover:pl-8'}`}
                            >
                                <User className="w-4 h-4" /> Profile Info
                            </button>
                            <button 
                                onClick={() => setActiveTab('security')}
                                className={`w-full text-left px-6 py-4 flex items-center gap-3 transition-all ${activeTab === 'security' ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-white text-slate-800 border border-slate-200 hover:border-blue-600 hover:pl-8'}`}
                            >
                                <Key className="w-4 h-4" /> Security
                            </button>
                            <button 
                                onClick={() => setActiveTab('preferences')}
                                className={`w-full text-left px-6 py-4 flex items-center gap-3 transition-all ${activeTab === 'preferences' ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-white text-slate-800 border border-slate-200 hover:border-blue-600 hover:pl-8'}`}
                            >
                        ...
                        <div className="pt-8 mt-auto">
                            <button 
                                ...
                                className="w-full text-left px-6 py-4 flex items-center gap-3 transition-all bg-white text-slate-800 border border-slate-200 hover:text-slate-800 hover:border-gray-500 hover:pl-8"
                            >
                    ...
                    <div className="md:col-span-3 space-y-12">
                        ...
                        {activeTab === 'profile' && (
                            <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <section className="bg-white border border-slate-200 p-8 md:p-12 hover:border-blue-600 transition-colors duration-500">
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div>
                                            <label className="block text-xs font-bold text-slate-800 uppercase tracking-widest mb-2">Registered Email</label>
                                            <div className="w-full border border-slate-200 bg-white px-4 py-4 text-slate-800 font-sans text-lg cursor-not-allowed opacity-80 decoration-dashed underline underline-offset-8">
                        ...
                        {activeTab === 'security' && (
                            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <section className="bg-white border border-slate-200 p-8 md:p-12">
                                    ...
                                    <form onSubmit={handlePasswordUpdate} className="space-y-6">
                                        <div>
                                            <label className="block text-xs font-bold text-slate-800 uppercase tracking-widest mb-2">Current Password</label>
                                            <input 
                                                type="password" 
                                                required
                                                value={pwdData.currentPassword}
                                                onChange={(e) => setPwdData({...pwdData, currentPassword: e.target.value})}
                                                className="w-full border border-slate-200 bg-white px-4 py-4 text-slate-800 font-sans focus:outline-none focus:border-blue-600 transition-colors" 
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-bold text-slate-800 uppercase tracking-widest mb-2">New Password</label>
                                            <input 
                                                type="password" 
                                                required
                                                minLength="6"
                                        ...
                                        <button 
                                            ...
                                            className="w-full sm:w-auto bg-blue-600 text-white hover:bg-blue-700 font-bold uppercase tracking-widest text-xs px-8 py-4 hover:bg-blue-700 transition-colors disabled:opacity-50"
                                        >
                            ...
                            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <section className="bg-white border border-slate-200 p-8 md:p-12">
                                    ...
                                    <div className="grid grid-cols-1 gap-6">
                                        ...
                                        <div className="border border-slate-200 p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 cursor-pointer hover:border-gray-600 transition-colors" onClick={handleStrictFilterToggle}>
                                            <div>
                                                <h3 className="font-bold text-lg mb-1 flex items-center gap-2">Strict AI Formatting <EyeOff className="w-4 h-4 text-slate-800"/></h3>
                                                <p className="text-sm text-slate-800">
                                                    Aggressive heuristic shielding. Re-calibrates the toxicity analyzer to maximize threat detection precision.
                                                </p>
                                            </div>
                                            <div className="shrink-0 flex items-center gap-3">
                                                <span className={`text-xs font-bold uppercase tracking-widest ${user?.strictAIFilter ? 'text-green-500' : 'text-slate-600'}`}>
                                                    {filterUpdating ? 'Saving...' : user?.strictAIFilter ? 'Enabled' : 'Disabled'}
                                                </span>
                                                <div className={`w-12 h-6 rounded-full flex items-center px-1 transition-colors ${user?.strictAIFilter ? 'bg-blue-600' : 'bg-gray-800'}`}>
