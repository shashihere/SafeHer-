...
const Register = () => {
    ...
    return (
        <div className="flex-grow flex items-center justify-center p-4 min-h-[calc(100vh-80px)] py-12 relative overflow-hidden bg-[#fdfdfd]">
            <div className="w-full max-w-lg bg-white border-8 border-black p-8 md:p-14 relative z-10 shadow-[12px_12px_0px_rgba(0,0,0,1)] hover:-translate-y-1 hover:-translate-x-1 hover:shadow-[16px_16px_0px_rgba(0,0,0,1)] transition-all duration-300">
                ...
                <div className="relative z-10">
                    <div className="flex justify-center mb-8">
                        <div className="p-4 bg-green-400 border-4 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] inline-block rotate-3 hover:rotate-0 transition-transform">
                            <ShieldCheck className="w-12 h-12 text-black" />
                        </div>
                    </div>
                    <h2 className="text-5xl font-black font-cursive text-center mb-2 tracking-tighter text-black uppercase">Join Raksha</h2>
                    <p className="text-center text-slate-800 mb-10 font-sans tracking-widest uppercase text-xs font-bold bg-gray-100 py-2 border-y-2 border-black">Your safety matters. Create a secure vault.</p>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div>
                            ...
                            <input 
                                type="text" 
                                className="w-full bg-white border-4 border-black px-4 py-4 text-black focus:outline-none focus:bg-green-100 transition-colors placeholder:text-slate-400 font-bold shadow-[inset_0_2px_4px_rgba(0,0,0,0.1)]"
                                value={name}
                        ...
                        <div className="pt-6">
                            <button disabled={loading} type="submit" className="w-full bg-blue-600 text-white font-black py-5 px-4 border-4 border-black shadow-[6px_6px_0px_rgba(0,0,0,1)] hover:-translate-y-1 hover:shadow-[10px_10px_0px_rgba(0,0,0,1)] hover:bg-blue-500 transition-all uppercase tracking-widest text-lg flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                                {loading ? <Loader2 className="w-6 h-6 animate-spin text-white" /> : 'CREATE IDENTITY'}
                            </button>
                        </div>
                    </form>
                    
                    <div className="mt-10 pt-6 text-center border-t-4 border-black">
                        <p className="text-black uppercase tracking-widest text-sm font-bold bg-green-400 inline-block px-4 py-2 border-2 border-black">
                            Already have an account? 
                            <Link to="/login" className="ml-2 text-blue-700 hover:text-blue-900 transition-colors underline decoration-2 underline-offset-4">Log in</Link>
                        </p>
                    </div>
                </div>
            </div>
